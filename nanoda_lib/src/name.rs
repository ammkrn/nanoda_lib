...
impl<'a> Name<'a> {
    ...
    pub fn insert_env<'e>(self, env : &mut Env<'e, impl 'e + IsTracer>, live : &Store<LiveZst>) -> NamePtr<'e> {
        match self {
            ...
            Str(pfx, sfx) => {
                ...
                match sfx {
                    ...
                    Ptr::P(..) => unreachable!("P strings should not exist")
                }
            },
...
impl<'a> NamePtr<'a> {
    ...
    // there is no step; you just trace the result unless
    // this constructor has already been called within either
    // the environment, or this local context
