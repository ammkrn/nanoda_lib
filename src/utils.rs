...
use crate::env::{Declar, Notation, RecRule};
use crate::expr::{BinderStyle, Expr, ExprPtr, LocalSerial};
use crate::level::{Level, Level::*, LevelsPtr};
use crate::name::{Name, Name::*, NamePtr};
...
impl<'t, 'l: 't, 'e: 'l> Tc<'t, 'l, 'e> {
    ...
    pub fn replace_local(&mut self, l: ExprPtr<'l>) {
        match l.read(self) {
            Expr::Local { b_type, .. } => match self.local_cache.get_mut(&b_type) {
                Some(v) => {
                    v.push(l);
                }
                None => {
                    self.local_cache.insert(b_type, vec![l]);
                }
            },
...
pub trait IsStored<'x>: Sized {
    type Carrier<'a>: Eq + Hash;
    fn mut_map<'a, C: IsCtx<'a>>(ctx: &mut C) -> &mut Set<Self::Carrier<'a>, C::Writable>;
    fn read2<'a, S: IsCtx<'a>>(self, storage: &S) -> Self::Carrier<'a>
    where
        'x: 'a;
    fn alloc2<'a, S: IsCtx<'a>>(self, storage: &mut S) -> Ptr<'a, Self::Carrier<'a>>
    where
        'x: 'a;
}

impl<'x> IsStored<'x> for Level<'x> {
    type Carrier<'a> = Level<'a>;

    fn mut_map<'a, C: IsCtx<'a>>(ctx: &mut C) -> &mut Set<Self::Carrier<'a>, C::Writable> {
        &mut ctx.mut_store().levels
    }

    /// no-op since we already have the actual data.
    fn read2<'a, S: IsCtx<'a>>(self, _: &S) -> Self::Carrier<'a>
    where
        'x: 'a,
    {
        self
    ...
    /// Check whether this data is already allocated, checking
    /// the longest lived arena first. If it's not a duplicate,
    /// allocate it in the arena we currently have mutable access to,
        ...
        'x: 'a,
    {
        storage
            ...
            .or_else(|| {
                storage
                    .live_store()
                    .and_then(|sto| sto.levels.check_dupe(&self))
            })
            .unwrap_or_else(|| {
                let x: Level<'a> = self.read2(storage);
                <Self as IsStored>::mut_map(storage).insert_elem(x)
            })
    }
}

impl<'x> IsStored<'x> for Ptr<'x, Level<'x>> {
    type Carrier<'a> = Level<'a>;
    ...
    /// Get the pointed-to data out of the proper arena.
    fn read2<'a, S: IsCtx<'a>>(self, storage: &S) -> Self::Carrier<'a>
    where
        'x: 'a,
    {
        match self {
            Ptr::E(index, h, z) => *storage
                .env_store()
                .levels
                .get_elem(index, h, z),
            Ptr::L(index, h, z) => *storage
                .live_store()
    ...
    /// no-op since this is already a pointer.
    fn alloc2<'a, S: IsCtx<'a>>(self, _: &mut S) -> Ptr<'a, Self::Carrier<'a>>
    where
        'x: 'a,
    {
        self
...
impl<'l, 'e: 'l> IsLiveCtx<'l> for Live<'l, 'e> {
    ...
    fn next_local(&mut self) -> LocalSerial {
        match self {
            Compiler { next_local, .. } | Checker { next_local, .. } => {
...
impl<'s, 'a: 's> Ptr<'a, String> {
    pub fn read(self, ctx: &'s impl IsCtx<'a>) -> &'s String {
        match self {
            ...
            Ptr::L(index, h, z) => &ctx
                .live_store()
                .expect("Strings failed to read live store!")
                .strings
                .get_elem(index, h, z),
        }
    }
...
macro_rules! arena_item {
    ( $short:lifetime, $env:lifetime, $live:lifetime, $base:ident, $field:ident ) => {
        impl<$short> $base<$short> {
            pub fn check_dupe(
                self,
                ctx: &impl IsCtx<$short>,
            ) -> Option<Ptr<$short, $base<$short>>> {
                ctx.env_store().$field.check_dupe(&self).or_else(|| {
                    ctx.live_store()
                        .and_then(|sto| sto.$field.check_dupe(&self))
                })
            }
        ...
        impl<$short> Ptr<$short, $base<$short>> {
            ...
            pub fn insert_env<$live, $env>(
                self,
                env: &mut Env<$env>,
                live: &Store<$live, LiveZst>,
            ) -> Ptr<$env, $base<$env>> {
                match self {
                    Ptr::E(index, _, z) => {
                    ...
                    Ptr::L(index, h, z) => live.$field.get_elem(index, h, z).insert_env(env, live),
                }
            }
        }
    ...
    ( $short:lifetime, $env:lifetime, $live:lifetime, $base:ident, $list_field:ident ) => {
        impl<$short> List<$short, $base<$short>> {
            ...
            pub fn check_dupe(
                self,
                ctx: &impl IsCtx<$short>,
            ) -> Option<ListPtr<$short, $base<$short>>> {
                ctx.env_store().$list_field.check_dupe(&self).or_else(|| {
                    ctx.live_store()
                        .and_then(|sto| sto.$list_field.check_dupe(&self))
                })
            }
        ...
        impl<$short> ListPtr<$short, $base<$short>> {
            ...
            pub fn skip(
                self,
                n: usize,
                ctx: &impl IsLiveCtx<$short>,
            ) -> ListPtr<$short, $base<$short>> {
                match self.read(ctx) {
                    _ if n == 0 => self,
                    Nil => self,
